...
2.  **画面简述标准（关键，必须严格遵守）：**
    * **禁止空泛**：不要写“唯美/高级/震撼/神秘”等抽象词替代内容，必须写清楚镜头里能看到的“物体、材质、光线、运动”。  
    * **背景细节必须爆炸丰富（硬指标）**：在 [环境/背景细节] 中必须给出 **至少 6 个**具体可见元素（用“、”分隔），并且至少包含：
      - **前景元素 ≥ 2**（例如：水珠挂在金属边缘、漂浮的细尘、镜头边缘的枝叶剪影）
      - **中景元素 ≥ 2**（例如：建筑、人物/生物的局部、道具、符号）
      - **远景元素 ≥ 2**（例如：天际线、山脉、巨构、极光、云层结构）
    * **主题锚点防跑题（硬指标）**：每个镜头必须显式包含 [主题锚点符号]，其中至少 2 个符号/意象必须与主题“${topic}”强相关（例如：概念具象化的符号、文化符号、关键道具、重复出现的图腾/标记）。
    * **一致性与递进（硬指标）**：
      - 如果已有分镜，则本镜头必须 **继承至少 1 个**已出现的视觉母题/背景元素（保持世界观连续），并在此基础上 **新增 1 个**更强烈的新视觉亮点（实现递进）。
    ...
    * **材质与光影（硬指标）**：[光影/材质] 至少包含 **3 个**材质/光影细节（如“湿润金属拉丝反光、体积雾、轮廓光、丁达尔光束、冷暖对比”）。
...
**JSON结构模版（可在 outline 每个镜头中额外补充字段，以增强稳定性）：**
{
    ...
    "outline": [
        {
            "id": ${index + 1},
            "summary": "必须使用结构化格式输出： [主体]... + [环境/背景细节(≥6个元素，含前/中/远景)]... + [关键动作/动态(≥2)]... + [运镜/镜头语言(含景深/焦点)]... + [光影/材质(≥3)]... + [主题锚点符号(≥2)]... + [情感暗示]...",
            ...
            "background_elements": ["至少6个具体背景元素（字符串数组，尽量覆盖前/中/远景）"],
            "recurring_motifs": ["需要在后续镜头重复出现的母题/符号（2-4个）"]
        }
    ]
}`;

    const resultRaw = await callDeepSeek(apiKey, systemPrompt, "开始策划", 4096, 120000, abortSignal);
...
請输出JSON：
{
    ...
    "voiceover": "中文旁白；若选择无旁白则返回空字符串\"\"。"
        ...
        const timeoutMs = detailPreset === 'pro' ? 120000 : 60000;
        const resultRaw = await callDeepSeek(apiKey, systemPrompt, userPrompt, maxTokens, timeoutMs, abortSignal);
        const result = parseJsonResult(resultRaw);
...
async function optimizeSceneSummary(index) {
    ...
    try {
        ...
        const systemPrompt = `你是一位专业电影分镜师。你的任务是将用户提供的“画面描述”优化为一段具有强视觉张力、丰富光影细节和明确镜头感的专业分镜规划。

核心要求：
1. **结构化输出**：必须严格保留或重构为以下格式（方括号不可省略）：
   [主体] ... + [环境/背景] ... + [关键动作] ... + [运镜/光影] ...
2. **Sora风格增强**：增加材质细节（如“粗糙的纹理”、“反光的表面”）、光影描述（如“丁达尔效应”、“冷暖对比”）和动态感。
3. **指令响应**：如果用户提供了修改指令（如“改成下雪”），必须优先执行该指令，同时保持上述结构。
4. **简洁有力**：字数控制在 80-150 字，不要啰嗦。
5. **只输出正文**：不要加“好的”、“优化后”等废话。`;

        const userPrompt = `原规划：${currentSummary}
${userInput ? `用户修改指令：${userInput}` : '请在保持原意基础上，增强画面细节和镜头感。'}
    ...
    try {
        ...
        const doFetch = async (withResponseFormat) => {
            ...
            const ms = typeof timeoutMs === 'number'
                ? timeoutMs
                : (maxTokens >= 4096 ? 120000 : 60000); // 默认：长输出给更长超时
...
function parseJsonResult(content) {
    try {
        ...
        if (start !== -1 && end !== -1) return JSON.parse(content.substring(start, end));
        throw e;
    }
}
